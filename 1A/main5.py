...
def classify_headings_multilingual(spans):
    """Improved heading detection using structure, layout and numbering"""
    headings = []
    seen = set()
    ...
    for span in spans:
        ...
        # Skip noise
        if (not text or is_page_number(text) or is_footer_header(text)):
            continue

        # Remove junk: lines of dashes, single punctuation, etc.
        if re.fullmatch(r'[-_=~.]{4,}', text):
            continue

        # Filter out form-like fields (like 'Name', 'Age')
        if len(text.split()) <= 2 and not re.match(r'\d', text) and text.lower() in ["name", "age", "relationship", "date", "signature"]:
            continue

        # Heading classification
        level = detect_level_from_numbering(text)
        if level:
            headings.append({
                "level": level,
                "text": text,
        ...
        elif is_visually_prominent(span):
            headings.append({
                "level": "H3",
                "text": text,
                "page": span['page']
    ...
    # Remove headings that are just numbers
    headings = [h for h in headings if not re.fullmatch(r'\d+[.]?', h['text'].strip())]

    # Sort
    return sorted(headings, key=lambda x: (x['page'], x['text']))

def detect_level_from_numbering(text):
    """Assign H1/H2/H3 based on numeric heading format"""
...
def is_visually_prominent(span):
    """Font size + bold + title-casing as fallback"""
    is_bold = span['flags'] & 2**4
    is_large = span['size'] > 14
    is_titled = span['text'].istitle()
    return is_bold or is_large or is_titled
