...
class SmartPDFOutlineExtractor:
    def __init__(self):
        ...
        self.address_patterns = [
            ...
            r'^WWW\.[A-Z0-9]+\.[A-Z]+$',  # "WWW.TOPJUMP.COM"
            r'^RSVP:\s*[-_]+$',  # "RSVP: ----------------"
        ]
            ...
            for element in layout:
                if isinstance(element, LTTextContainer):
                    for line in element:
                        ...
                        common_font = Counter(font_names).most_common(1)[
                            0][0] if font_names else ""
    ...
    def is_likely_heading(self, element, doc_stats):
        ...
        # Special case: if it looks like a document title, it should be a heading
        if any(word in text.lower() for word in ['application', 'form', 'overview', 'document', 'report']):
            score += 2
        ...
        # Special handling for promotional/invitation text
        if any(word in text.upper() for word in ['HOPE', 'SEE', 'THERE']) and len(text) > 10:
            score -= 1  # Lower score for promotional text
        ...
        if re.match(r'^WWW\.', text.upper()):  # URLs
            score -= 2

        if re.match(r'^RSVP:', text.upper()):  # RSVP lines
            score -= 1
    ...
    def extract_title(self, elements, headings):
        ...
        for elem in elements[:15]:  # Check first 15 elements
            ...
            # Skip very short text unless it's clearly a title
            if len(text) < 8:
                continue

            # Look for title-like content
            is_title_like = (
                # Contains key title words
                any(word in text.lower() for word in ['form', 'application', 'overview', 'document', 'report', 'manual', 'guide']) or
                # Multi-word substantial text
                (len(text.split()) >= 3 and len(text) > 15) or
                # Looks like a formal title (title case, reasonable length)
                (text.istitle() and 20 <= len(text) <= 100)
            )

            # Not numbered section
            if is_title_like and not re.match(r'^\d+\.', text):
        ...
        h1_headings = [h for h in headings if h['level']
                       == 'H1' and len(h['text'].strip()) > 8]
        if h1_headings:
            return h1_headings[0]['text'].strip()

        # Last resort: any first heading
        if headings and len(headings[0]['text'].strip()) > 5:
    ...
    def process_document(self, pdf_path):
        ...
        try:
            ...
            # Identify headings
            heading_candidates = []
            title_text = None  # Track potential title
            ...
            for element in elements:
                if self.is_likely_heading(element, doc_stats):
                    ...
                    # Track potential title (first substantial heading)
                    if title_text is None and len(element['text'].strip()) > 10:
                        title_text = element['text'].strip()

            # Special case: ensure the document title appears as a heading
            title = self.extract_title(elements, heading_candidates)

            # If title is not already in headings, add it
            title_in_headings = any(h['text'].strip().lower(
            ) == title.lower() for h in heading_candidates)
            if title and len(title.strip()) > 8 and not title_in_headings:
                # Find the title element to get its properties
                for element in elements[:10]:
                    if element['text'].strip().lower() == title.lower():
                        heading_candidates.insert(0, {
                            'level': 'H1',
                            'text': title.strip(),
                            'page': element['page'],
                            'font_size': element['font_size']
    ...
    def deduplicate_and_limit(self, headings):
        ...
        for heading in headings:
            ...
            final_heading = {k: v for k,
                             v in heading.items() if k != 'font_size'}
...
def main():
    ...
    for filename in pdf_files:
        ...
        for i, heading in enumerate(result['outline'][:5]):  # Show first 5
            print(f"    {heading['level']}: {heading['text'][:60]}...")

        if len(result['outline']) == 0:
            print("  ! No headings found - this might indicate over-filtering")
